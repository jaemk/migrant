...
- Migration tags must all be unique and may only contain the characters `[a-z0-9-]`.
  ...
  See the [embedded_cli_compatible](https://github.com/jaemk/migrant/blob/main/migrant_lib/examples/embedded_cli_compatible.rs)
  example.
- Function migrations must have the signature `fn(ConnConfig) -> Result<(), Box<dyn std::error::Error>>`.
  See the [embedded_programmable](https://github.com/jaemk/migrant/blob/main/migrant_lib/examples/embedded_programmable.rs)
...
See the [migrant_cli_compatible](https://github.com/jaemk/migrant/blob/main/migrant_lib/examples/migrant_cli_compatible.rs)
...
/// editor), print human-readable status to stdout, so they are kept out of the
/// crate root and grouped here.
pub mod cli {
    pub use crate::ops::{edit, list, shell};
}
